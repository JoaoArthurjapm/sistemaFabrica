...
// Fun��o para obter informa��es do fabricante
...
// Fun��o para obter informa��es do produto
struct Produto obterProduto(int produtoNum, const struct Fabricante *fabricantes, int numFabricantes)
{
    ...
    // Obter o n�mero do fabricante do produto
    int numFabricante;
    printf("N�mero do Fabricante (1-%d): ", numFabricantes);
...
// Fun��o para exibir as informa��es dos produtos em formato de tabela
void exibirProdutos(const struct Produto *produtos, int numProdutos)
{
    int i;
    printf("\nInforma��es dos produtos:\n");
    printf("+------+-------------------------+--------+--------------+--------------+---------+---------+\n");
    printf("|  N�  |       Descri��o         |  Peso  | Valor Compra | Valor Venda  |  Lucro  |  %% Lucro |\n");
    printf("+------+-------------------------+--------+--------------+--------------+---------+---------+\n");
...
// Fun��o para exibir o menu e executar a op��o selecionada
void exibirMenu(const struct Produto *produtos, int numProdutos, const struct Fabricante *fabricantes, int numFabricantes)
{
    ...
    do
    {
        ...
        printf("[5] Apresentar o(s) estado(s) onde est� registrado o produto mais caro\n");
        printf("[6] Apresentar o(s) fabricante(s) onde est� registrado o produto mais barato\n");
        ...
        case 5:
            // Apresentar o(s) estado(s) onde est� registrado o produto mais caro
            {
                ...
                printf("\nEstado(s) onde est� registrado o produto mais caro:\n");
                for (i = 0; i < numProdutos; i++)
                {
                    if (produtos[i].valorVenda == maiorValor)
                    {
        ...
        case 6:
            // Apresentar o(s) fabricante(s) onde est� registrado o produto mais barato
            {
                ...
                printf("\nFabricante(s) onde est� registrado o produto mais barato:\n");
                for (i = 0; i < numProdutos; i++)
                {
                    if (produtos[i].valorVenda == menorValor)
                    {
...
int main()
{
    ...
    // Obter o n�mero de fabricantes
    do
    {
        printf("Digite o n�mero de fabricantes (2-%d): ", MAX_FABRICANTES);
        scanf("%d", &numFabricantes);
        while (getchar() != '\n')
            continue;
    } while (numFabricantes < 1 || numFabricantes > MAX_FABRICANTES);

    // Obter informa��es dos fabricantes
    for (i = 0; i < numFabricantes; i++)
    {
        fabricantes[i] = obterFabricante(i + 1);
    }

    // Obter o n�mero de produtos
    do
    {
        printf("\nDigite o n�mero de produtos (5-%d): ", MAX_PRODUTOS);
        scanf("%d", &numProdutos);
        while (getchar() != '\n')
            continue;
    } while (numProdutos < 1 || numProdutos > MAX_PRODUTOS);

    // Obter informa��es dos produtos
    for (i = 0; i < numProdutos; i++)
    {
        produtos[i] = obterProduto(i + 1, fabricantes, numFabricantes);
    }

    // Exibir as informa��es dos produtos em formato de tabela
    // exibirProdutos(produtos, numProdutos);

    // Exibir o menu e executar a op��o selecionada
